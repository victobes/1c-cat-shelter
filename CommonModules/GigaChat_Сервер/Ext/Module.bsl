...
Функция СоздатьСообщениеСКартинкой (ИдентификаторДиалога = Неопределено, Содержимое = Неопределено, АдресКартинки = Неопределено) Экспорт
	...
	Соединение = ПолучитьСоединение();
	///////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Удаление всех файлов из хранилища перед загрузкой фотографии
	Заголовки = Новый Соответствие;
	...
	// 
	HTTPЗапросПолучениеФайлов = Новый HTTPЗапрос("/api/v1/files", Заголовки);
	ОтветHTTPФайлы = Соединение.ВызватьHTTPМетод("GET", HTTPЗапросПолучениеФайлов);
	
	ОтветHTTPФайлы.ПолучитьТелоКакСтроку();
	
	ИДФайловДляУдаления = Новый Массив;
	
	Если ОтветHTTPФайлы.КодСостояния = 200 Тогда
		ЧтениеJSON = Новый ЧтениеJSON;
		ЧтениеJSON.УстановитьСтроку(ОтветHTTPФайлы.ПолучитьТелоКакСтроку());
		ДанныеJSON = ПрочитатьJSON(ЧтениеJSON);
		ЧтениеJSON.Закрыть();
		//
		МассивФайловВХранилище = ДанныеJSON.data;
		Если МассивФайловВХранилище.Количество() > 0 Тогда
			Для каждого Файл ИЗ МассивФайловВХранилище Цикл
				ИДФайловДляУдаления.Добавить(Файл.id);
			КонецЦикла;
		КонецЕсли;
		//
	Иначе
		...
		СтруктураСобытий.Комментарий 		 = "Ответ сервера (получение файлов): " + ОтветHTTPФайлы.КодСостояния + " " + ОтветHTTPФайлы.ПолучитьТелоКакСтроку();
	...
	УспешноУдаленные = Новый Массив;
	
	Для каждого ИДФайла Из ИДФайловДляУдаления Цикл 
		АдресРесурса = СтрШаблон("api/v1/files/%1/delete", ИДФайла); 
		HTTPЗапросУдаленияФайла = Новый HTTPЗапрос(АдресРесурса, Заголовки);
		ОтветHTTPУдаление = Соединение.ВызватьHTTPМетод("POST", HTTPЗапросУдаленияФайла);
	
		ОтветHTTPУдаление.ПолучитьТелоКакСтроку();
	
		Если ОтветHTTPУдаление.КодСостояния = 200 Тогда 
			ЧтениеJSON = Новый ЧтениеJSON;
			ЧтениеJSON.УстановитьСтроку(ОтветHTTPУдаление.ПолучитьТелоКакСтроку());
			ДанныеJSON = ПрочитатьJSON(ЧтениеJSON);
			ЧтениеJSON.Закрыть();
			
			УспешноУдаленные.Добавить(ДанныеJSON.id);
		Иначе
			СтруктураСобытий = Новый Структура("ИмяСобытия, ПредставлениеУровня, Комментарий, ДатаСобытия");

			СтруктураСобытий.ИмяСобытия 		 = "GigaChat: Удаление файла из хранилища";
			
			СтруктураСобытий.ПредставлениеУровня = "Ошибка";	//"Информация", "Ошибка", "Предупреждение", "Примечание".
			СтруктураСобытий.Комментарий 		 = СтрШаблон("Не удалось удалить файл с ИД = %1", ИДФайла);
	...
	СтруктураСобытий.ИмяСобытия 		 = "GigaChat: Результат удаления файлов";
	
	СтруктураСобытий.ПредставлениеУровня = "Информация";	//"Информация", "Ошибка", "Предупреждение", "Примечание".
	СтруктураСобытий.Комментарий 		 = СтрШаблон("Удалено файлов: %1", УспешноУдаленные.Количество());
